#!/usr/bin/env python3
"""
Analytical regression test for plane-wave model 2.
"""
...
def build_expectations(
    ...
    gradients["grad_phi2_x"] = (
        -(2.0 / 3.0) * (kmode ** 3) * cos_x * sin_y * sin_z * g2
    )

    gradients["grad_phi3a_x"] = (
        -(1.0 / 10.0) * (kmode ** 5) * cos_x * (cos_2y + cos_2z) * g3a
    )

    term1 = cos_x * (90.0 - 81.0 * cos_2z + cos_2y * (-81.0 + 10.0 * cos_2z))
    term2 = 60.0 * sin_2x * sin_y * sin_z
    gradients["grad_phi3b_x"] = (
        (1.0 / 540.0) * (kmode ** 5) * (term1 + term2) * g3b
    )

    gradients["curl_A3c_x"] = (
        (1.0 / 27.0)
        * (kmode ** 5)
        * (4.0 * cos_x * cos_2y * cos_2z - 3.0 * sin_2x * sin_y * sin_z)
        * g3c
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate plane-wave model 2 output.")
    ...
    print("Plane-wave model 2 diagnostics match analytical expectations.")
